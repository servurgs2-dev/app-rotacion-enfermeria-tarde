...
-- Alinea la defensa remota con el clasificador cliente de dependencias.
...
begin
  ...
  -- tambiÃ©n impide INSERT concurrente de una fila faltante para un tercer turno.
  ...
  -- InspecciÃ³n acotada de referencias legacy conocidas de Planilla y de los
  ...
  loop
    ...
    if jsonb_typeof(v_planilla) = 'object' then
      ...
      -- InspecciÃ³n acotada de las tres estructuras de rotaciÃ³n nocturna.
      -- Un personaId/id explÃ­cito siempre gana: los helpers sÃ³lo consideran legacy.
    ...
    if jsonb_typeof(v_calendario) = 'object' then
        ...
        loop
          if jsonb_typeof(v_periodo.value) = 'array' then
            ...
            loop
              ...
              if private.movimiento_padron_referencia_id(v_referencia_json) = v_persona_id then
                if not (
                  jsonb_typeof(v_referencia_json) = 'object'
                  and btrim(coalesce(v_referencia_json ->> 'personaId', '')) = v_persona_id
                  and btrim(coalesce(v_referencia_json ->> 'motivo', '')) in (
                    'falta_con_aviso',
                    'supervision_otro_turno',
                    'adhesion_paro',
                    'otro'
                  )
                  and btrim(coalesce(v_referencia_json ->> 'personaCoberturaId', '')) = ''
                  and coalesce(v_referencia_json -> 'vinculacionCambioOtroTurno' = 'true'::jsonb, false) = false
                ) then
                  raise exception using
                    message = 'REFERENCIAS_CALENDARIO_LOCAL_PENDIENTES',
              ...
              elsif btrim(coalesce(v_referencia_json ->> 'personaCoberturaId', '')) = v_persona_id
                or private.movimiento_padron_referencia_legacy_coincide(
                  v_referencia_json, v_persona
                ) then
                raise exception using
      ...
      if jsonb_typeof(v_calendario -> 'extras') = 'object' then
        for v_periodo in select key, value from jsonb_each(v_calendario -> 'extras')
        loop
          if jsonb_typeof(v_periodo.value) = 'array' then
            for v_referencia_json in select value from jsonb_array_elements(v_periodo.value)
            loop
              if (
                coalesce(v_referencia_json -> 'vinculacionCambioOtroTurno' = 'true'::jsonb, false)
                or btrim(coalesce(v_referencia_json ->> 'personaCubiertaId', '')) <> ''
              ) and (
                private.movimiento_padron_referencia_id(v_referencia_json) = v_persona_id
                or btrim(coalesce(v_referencia_json ->> 'personaCubiertaId', '')) = v_persona_id
                or private.movimiento_padron_referencia_legacy_coincide(
                  v_referencia_json, v_persona
                )
              ) then
                ...
                raise exception using
                  message = 'REFERENCIAS_CALENDARIO_LOCAL_PENDIENTES',
                  detail = v_estado.turno || '/extras/' || v_periodo.key,
                  errcode = 'P0001';
              end if;
            end loop;
          end if;
