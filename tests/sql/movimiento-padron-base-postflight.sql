-- READ-ONLY. Ejecutar después de aplicar la migración.
with funcion as (
  select
    p.oid,
    p.proowner,
  ...
  union all select 'tabla_principal', to_regclass('public.estado_por_turno_mes') is not null
  ...
  union all select 'helper_supervision', to_regprocedure('private.usuario_app_es_supervision()') is not null
  union all select 'rls_habilitada', coalesce((
    select relrowsecurity from pg_class where oid = 'public.estado_por_turno_mes'::regclass
  ), false)
)
select
  case when bool_and(ok) then 'POSTFLIGHT_OK' else 'REVISAR_POSTFLIGHT' end as resultado,
