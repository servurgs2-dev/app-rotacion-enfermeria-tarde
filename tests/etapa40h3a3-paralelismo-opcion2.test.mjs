...
const persona = (id, nombre, turnante = false) => ({
  id,
  nombre,
  ...(turnante ? { esTurnante: true } : {})
});
const comunes = [
...
const asignaciones = comunes.map(([sectorId, enfermero]) => ({
  nombre: filasPorId.get(sectorId)?.etiqueta || { sillones_3: "SILLONES 3" }[sectorId],
  sectorId,
  enfermero,
  tipo: "sector"
}));
...
const destino = (resultado, nombre) => resultado.asignaciones.find(
  (fila) => normalizar(fila.nombre) === normalizar(nombre)
);
let total = 0;
...
const paralelismo = [
  ...
  ["1–3 + 21 y 22", "carolina"], ["4–7 + 30", "sergio"],
  ["8–14", "laura"], ["15–20", "estefani"], ["DX 23–29", "romina"],
...
probar("Día 9 conserva identidad por sector y aplica los cinco grupos de opción 2", () => {
  for (const [nombre, personaId] of paralelismo) {
    assert.equal(destino(resultado, nombre)?.enfermero?.id, personaId, nombre);
  }
});

probar("20+22–24 se anula y Jhoana pasa al pool Turnante", () => {
  assert.equal(resultado.asignaciones.some(
    (fila) => normalizar(fila.nombre) === normalizar("19-20+22-24")
  ), false);
  assert.deepEqual(
    resultado.asignaciones.filter((fila) => fila.tipo === "turnante")
      .map((fila) => fila.enfermero?.id).filter(Boolean),
    ["jhoana"]
  );
});

probar("Turnantes que ya cubren sectores acompañan su paralelo sin duplicarse", () => {
...
probar("la prioridad extrema no altera REA 2 ni los grupos estructurales", () => {
  assert.equal(destino(resultado, "REA 2").enfermero.id, "diego");
  assert.equal(destino(resultado, "PRE INT 2").enfermero.id, "gabriela");
  assert.equal(destino(resultado, "1–3 + 21 y 22").enfermero.id, "carolina");
});

probar("opción 2 alcanza punto fijo después de persistir y reconstruir", () => {
  ...
  const r1 = recalcularRedistribucionOpcion2Automatica({
    ...parametros,
    asignaciones: resultado.asignaciones
  });
  const r2 = recalcularRedistribucionOpcion2Automatica({ ...parametros, asignaciones: r1 });
...
probar("un movimiento manual conserva identidad, destino y procedencia", () => {
  const manual = redistribuirPorBoxes({
    asignaciones: asignaciones.map((fila) => fila.sectorId === "rea_2"
...
probar("un No Disponible no reaparece ni desplaza las demás identidades", () => {
  const sinLaura = redistribuirPorBoxes({
    asignaciones: asignaciones.map((fila) => fila.enfermero?.id === "laura"
      ? { ...fila, enfermero: null }
      : fila),
    ordenVisual,
    filasConfiguracion,
  ...
  assert.equal(destino(sinLaura, "8–14").enfermero, null);
  assert.equal(destino(sinLaura, "REA 2").enfermero.id, "diego");
  assert.equal(sinLaura.asignaciones.some((fila) => fila.enfermero?.id === "laura"), false);
});

probar("un vacío manual conserva destino y procedencia sin ser rellenado estructuralmente", () => {
  const vacio = redistribuirPorBoxes({
    asignaciones: asignaciones.map((fila) => fila.sectorId === "pre_int_2"
      ? { ...fila, enfermero: null, vacioManual: true }
      : fila),
    ordenVisual,
    filasConfiguracion,
    prioridadSectorIds
  });
  assert.equal(destino(vacio, "PRE INT 2").enfermero, null);
  assert.equal(vacio.procedencias[normalizar("PRE INT 2")], "manual");
});

console.log(`\n${total} pruebas conductuales del paralelismo de opción 2 aprobadas.`);
